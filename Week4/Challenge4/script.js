function generate_board() {
    ...
    // YOUR CODE GOES HERE
    let people = document.getElementById('friends');
    console.log(people);
    for (let i = 0; i < people.options.length; i++) {
        if (people.options[i].selected) {
            friends.push(people.options[i].value);
        }
    }
    ...
    // YOUR CODE GOES HERE
    let total_count = friends.length * fruits.length;
    // console.log(total_count);
    let cards = [];
    for (fruit of fruits) {
        for (friend of friends) {
            let path = "cards/" + fruit + "_" + friend + ".png";
            cards.push(path);
            cards.push(path);
        }
    }
    ...
    for (let row = 0; row < num_rows; row++) {
        ...
        for (let col = 0; col < num_cols; col++) {
            ...
            card = cards.pop(); // take out the last card of cards array
            console.log("card to be added: " + card);
            result_str += `<img src=${card}>`;
